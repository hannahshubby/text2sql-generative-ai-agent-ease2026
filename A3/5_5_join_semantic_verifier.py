...
class JoinSemanticVerifierAgent:
    ...
    def main(self, cfg, step1_result: Dict[str, Any], step5_result: Dict[str, Any]):
        user_query = step1_result.get("user_query", "")
        joins = step5_result.get("joins", [])
        
        if not joins:
            return step5_result # Nothing to verify

        # 1. Prepare Verification Context
        # We need to explain what each table and column represents to the LLM.
        # We'll use the join plan and any available metadata.
        verification_plan = {
            "user_intent": user_query,
            "proposed_joins": joins,
            "column_metadata_hint": "Note: Examine if 'from_col' and 'to_col' are conceptually the same entity (e.g., both are Employee IDs) or different (e.g., one is a Line ID and the other is a Person ID)."
        }
        ...
        # 'meta' 키가 없을 경우를 대비해 초기화
        if "meta" not in step5_result:
            step5_result["meta"] = {}
...
def main(cfg, step1_result, step5_result):
    verifier = JoinSemanticVerifierAgent(cfg)
    return verifier.main(cfg, step1_result, step5_result)
