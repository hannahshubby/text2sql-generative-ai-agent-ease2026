...
def main(cfg, summary_llm):
    ...
    for m in term_mentions:
        ...
        # Try to choose first; if an exact override exists, we keep it even if cand_count is huge.
        ...
        # Drop conditions apply ONLY when we did NOT hit exact override
        if used_rule != "EXACT_ORIGINALTERM_MATCH_OVERRIDE":
            if cand_count > MAX_CANDIDATES_DROP:
                dropped.append({**m, "dropReason": f"TOO_MANY_CANDIDATES>{MAX_CANDIDATES_DROP}"})
                continue
            if len(n_surface) <= MIN_SURFACE_LEN_KEEP and cand_count > 1:
                dropped.append({**m, "dropReason": f"SHORT_SURFACE_AMBIGUOUS(len<={MIN_SURFACE_LEN_KEEP})"})
                continue
