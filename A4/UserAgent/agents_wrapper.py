...
agent5_verifier = import_module_from_path("agent5_verifier", "5.5_join_semantic_verifier.py")
agent6 = import_module_from_path("agent6", "6.sql_planner_agent.py")
agent7 = import_module_from_path("agent7", "7.sql_synthesizer_agent.py")
...
def run_pipeline(user_query: str) -> str:
    ...
    # Ensure 'user_query' exists for downstream agents
    ...
    hierarchy = load_json(HIERARCHY_JSON) # Contains semantic_catalog
    join_graph = load_json(JOIN_GRAPH_JSON)
    col_to_tabs = load_json(COL_TO_TABLES_JSON)
    
    print(f"--- [Step 5] Table Linking ---")
    try:
        # A4 Step 5 expects: step1_result, step4_result, col_to_tabs, jgraph, hierarchy
        out5 = agent5_linking.main(out1, out4_min, col_to_tabs, join_graph, hierarchy)
        if "error" in out5 or "selected_tables" not in out5:
            raise ValueError(out5.get("error", "Linking returned empty results"))
    except Exception as e:
        print(f" [!] Step 5 Failed ({e}). Using DUMMY table structure.")
        out5 = {
            "root_anchor": "UNKNOWN_TABLE",
            "selected_tables": [{"table": "UNKNOWN_TABLE", "tier": 1, "role": "root"}],
            "joins": [],
            "column_bindings": {},
            "meta": {"fallback": True}
        }
    ...
    sem_cat = hierarchy.get("semantic_catalog", {})
    try:
        out5_verified = agent5_verifier.main(CFG, out1, out5, sem_cat)
    except Exception as e:
    ...
    try:
        out6 = agent6.main(CFG, out1, out4_min, out5_verified, sem_cat)
    except Exception as e:
    ...
    try:
        ...
        out7 = {"sql": "-- SQL Synthesis failed internally due to error"}
    
    final_sql = out7.get("sql", "Error: SQL not generated")
    print(f"\n[Generated SQL (Step 7)]\n{final_sql}")
    
    # Optional: Save results to files
    out_path = OUT_DIR / "final_sql.json"
    dump_json(out7, out_path)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("   SQL Generative AI Agent (A4 Pipeline)")
    print("="*50)
